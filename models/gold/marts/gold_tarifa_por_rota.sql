{{ config(
    materialized='table',
    file_format='delta',
    description='Tarifação vigente por rota com atributos completos de preço e transferência',
    tblproperties={
...
WITH tarifa_rota AS (
  SELECT
    ...
    CASE
      ...
      ELSE                                            'SPTrans'
    END                                           AS operador
  FROM {{ source('sptrans_raw', 'silver_fare_rules') }} fr
  JOIN {{ source('sptrans_raw', 'silver_fare_attributes') }} fa ON fr.fare_id  = fa.fare_id
  JOIN {{ source('sptrans_raw', 'silver_routes') }}           r  ON fr.route_id = r.route_id
)
SELECT
  *,
