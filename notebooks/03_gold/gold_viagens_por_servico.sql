-- =============================================================================
-- 4. gold_viagens_por_servico
--    Volume de viagens por tipo de serviço (dia útil, sábado, domingo),
--    com totais por linha e direção.
-- =============================================================================
CREATE OR REPLACE TABLE gold.sptrans.gold_viagens_por_servico
USING DELTA
COMMENT 'Consolidado de viagens por tipo de serviço, linha e direção'
TBLPROPERTIES (
...
WITH classificacao_servico AS (
  SELECT
    ...
    c.start_date,
    c.end_date
  FROM silver.sptrans.calendar c
),
viagens_agg AS (
  SELECT
    t.service_id,
  ...
  FROM silver.sptrans.trips         t
  JOIN silver.sptrans.routes        r  ON t.route_id   = r.route_id
...
FROM viagens_agg
ORDER BY tipo_servico, numero_linha, sentido;
